    ...
    return (
        ...
        <div className="bg-white/5 border border-white/10 rounded-xl overflow-hidden hover:border-white/20 transition group">
            ...
            <div className={`${gen?.aspectRatio === '9:16' ? 'aspect-9/16' : 'aspect-video'} relative overflow-hidden`} >
                    {gen.generatedImage && (<img src={gen.generatedImage} alt={gen.productName} className={`absolute inset-0 w-full h-full object-cover transition 
                        duration-500 ${gen.generatedVideo ? 'group-hover:opacity-0' : 'group-hover:scale-105'
                        }`}/>
                )}

                {gen.generatedVideo && (
                    <video src={gen.generatedVideo} className="absolute inset-0 w-full h-full object-cover opacity-0 transition duration-500 group-hover:opacity-100" playsInline loop muted 
                ...
                <div className="absolute left-3 top-3 gap-2 items-center">
                    {gen.isGenerating && (
                    <span className="bg-yellow-600/30 text-xs px-2 py-1 rounded-full">Generating</span>
                    )}
                    {gen.isPublished && (
                ...
                <div className="absolute right-3 bottom-3">
                    <img src={gen.uploadedImages[0]} alt="product" className="w-16 h-16 rounded-full object-cover animate-float" />
                    <img src={gen.uploadedImages[1]} alt="model" className="w-16 h-16 rounded-full object-cover animate-float -ml-8" style={{animationDelay:'3s'}} />
                </div>
            ...
            <div className="p-4">
                ...
                {gen.productDescription && (
                    <div className="mt-3">
                        ...
                        <div className="text-sm text-gray-300 bg-white/3 p-2 roundedmd wrap-break-word">{gen.productDescription}</div>
                    </div>
                )}
                {/* user prompt */}
                {gen.userPrompt && (
                    <div className="mt-3">
                        ...
                        <div className="text-xs text-gray-300 bg-white/3 p-2 roundedmd wrap-break-word">{gen.userPrompt}</div>
                    </div>
                )}
                {/* buttons */}
                {!forCommunity && (
                    <div className="mt-4 grid grid-cols-2 gap-3">
                        ...
                        <PrimaryButton onClick={()=> togglePublished(gen.id)}
